...
(function() {
    ...
    function isReadcacheLink(url) {
        return url.toLowerCase().includes("https://libmedium.batsense.net");
    }
        ...
        function replaceAllLinks(domain) {
            ...
            links.forEach(link => {
                ...
                if (href && href.startsWith('/')) {
                    link.href = "https://libmedium.batsense.net" + link.href;
                }
            });
        }
        function extractDomain(originalURL) {
            return originalURL.replace("medium.com", "libmedium.batsense.net");
        }
    // Main function
    (async function() {
        ...
        if (!isReadcacheLink(currentUrl)) {
            if (await isMediumPost(currentUrl)) {
                location.href = currentUrl.replace("medium.com", "libmedium.batsense.net");
            } else {
                console.log("The current webpage is not a Medium post.");
            }
        } else {
            console.log("The current URL is already a readcache link.");
            replaceAllLinks("https://libmedium.batsense.net" + extractDomain(location.href))
        }
    })();
})();
